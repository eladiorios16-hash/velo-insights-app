...
// --- RUTA DE CARGA MASIVA DE CALENDARIO (NUEVA) ---
app.get('/restore-calendar', async (req, res) => {
    try {
        // 1. Borramos el calendario anterior para evitar duplicados
        await db.query("TRUNCATE TABLE calendario");
        
        // 2. Insertamos las 32 carreras con sus etapas (Tu SQL exacto)
        const sql = `
        ...
        await db.query(sql);
        res.send("✅ Calendario completo (32 carreras) cargado correctamente.");
    } catch (e) { 
        console.error(e);
        res.status(500).send("❌ Error cargando calendario: " + e.message); 
    }
});
